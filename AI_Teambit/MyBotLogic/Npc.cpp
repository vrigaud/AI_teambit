...
void Npc::updateState()
{
    ...
    if (mObjective.mType == Objective::NONE || mObjective.mIsAchieved)
    {
        mNextState = WAITING;
    }
...
void Npc::searchPath()
{
    ...
    if (mPath.size() == 1)
    {
        ...
        BOT_LOGIC_NPC_LOG(mLogger, "\tduration : " + std::to_string(duration_cast<microseconds>(apres - avant).count()) + "us", true);
...
{
    BOT_LOGIC_NPC_LOG(mLogger, "- aStar", false);
    ...
    while (!openedList.empty())
    {
        ...
        for (size_t i{}; i < Node::NBNEIGHBOURS; ++i)
        {
            ...
            if (!neighbour || neighbour->getType() == Node::FORBIDDEN || neighbour->getType() == Node::OCCUPIED
